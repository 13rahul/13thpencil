(function(){
  ...
  function drawStrategyMarks(ctx, W, H, t, ink, era){
    ...
    for(var i=0;i<count;i++){
      ...
      ctx.strokeStyle = isThirteen ? era : ink;
      ctx.globalAlpha = isThirteen ? 1 : .42;
      ctx.lineWidth = isThirteen ? 4 : 2;
      ctx.beginPath();
      if(isThirteen){
        ctx.save(); ctx.translate(x, H*.5 + Math.sin(t*.02)*8); ctx.rotate(.28 + Math.sin(t*.015)*.12);
        ctx.moveTo(0,-H*.30); ctx.lineTo(0,H*.30); ctx.stroke(); ctx.restore();
      }else{
        ctx.moveTo(x, H*.22 + wob); ctx.lineTo(x, H*.78 + wob); ctx.stroke();
      }
    }
    ctx.globalAlpha = .18; ctx.strokeStyle = ink; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(W*.06,H*.5); ctx.lineTo(W*.94,H*.5); ctx.stroke();
    ctx.globalAlpha = 1;
  }
  ...
  if(loader && strokes){
    ...
    if(reduce){
      loader.hidden = true; if(hero) hero.classList.add("lit");
    }else{
  ...
  if(g){
    for(var r=0;r<3;r++){
      ...
      for(var c13=0;c13<13;c13++){
        var m = document.createElement("b");
        if(c13===12) m.className = "thirteen";
        row.appendChild(m);
      }
      g.appendChild(row);
    }
  ...
  var path = $("#processPath"), pathLen = 0, processSec = $("#process");
  if(path){
  ...
  function onScroll(){
    ...
    if(railTip) railTip.style.top = (p*100) + "%";
    if(path && processSec){
  ...
  var stage = $("#capStage"), canvas = $("#capCanvas");
  if(stage && canvas){
    ...
    function draw(){
      ...
      if(cap==="strategy"){
        drawStrategyMarks(c, W, H, t, ink, era);
      }
    ...
    function setCap(name){
      ...
      var capCaption = $("#capCaption");
      if(capCaption) capCaption.textContent = captions[name];
  ...
  /* ---------- 10b. BRAND & STRATEGY — twelve sit, the thirteenth refuses ---------- */
  var wall = $("#templateWall");
  if(wall){
    var outCell = $(".bs-cell--out", wall);
    if(outCell){
      if(reduce) outCell.classList.add("go");
      else setTimeout(function(){ outCell.classList.add("go"); }, 480);
    }
  }
  ...
  if(swapEl && !reduce && swaps.length > 1){
